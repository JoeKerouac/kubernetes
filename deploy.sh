...
execSSHWithoutPasswd "scp -r -P ${MASTER_SSH_PORT} ${APISERVER_COMPILE_TARGET_DIR} ${MASTER_SSH_USER}@${MASTER_IP}:${K8S_INSTALLER_DIR}" ${MASTER_SSH_PASSWD} & echo "${APISERVER_COMPILE_TARGET_DIR}分发完毕"
execSSHWithoutPasswd "scp -r -P ${MASTER_SSH_PORT} ${CM_COMPILE_TARGET_DIR} ${MASTER_SSH_USER}@${MASTER_IP}:${K8S_INSTALLER_DIR}" ${MASTER_SSH_PASSWD} & echo "${CM_COMPILE_TARGET_DIR}分发完毕"
execSSHWithoutPasswd "scp -r -P ${MASTER_SSH_PORT} ${SCHEDULER_COMPILE_TARGET_DIR} ${MASTER_SSH_USER}@${MASTER_IP}:${K8S_INSTALLER_DIR}" ${MASTER_SSH_PASSWD} & echo "${SCHEDULER_COMPILE_TARGET_DIR}分发完毕"
execSSHWithoutPasswd "scp -r -P ${MASTER_SSH_PORT} ${PROXY_COMPILE_TARGET_DIR} ${MASTER_SSH_USER}@${MASTER_IP}:${K8S_INSTALLER_DIR}" ${MASTER_SSH_PASSWD} & echo "${PROXY_COMPILE_TARGET_DIR}分发完毕"
execSSHWithoutPasswd "scp -r -P ${MASTER_SSH_PORT} ${KUBECTL_COMPILE_TARGET_DIR} ${MASTER_SSH_USER}@${MASTER_IP}:${K8S_INSTALLER_DIR}" ${MASTER_SSH_PASSWD} & echo "${KUBECTL_COMPILE_TARGET_DIR}分发完毕"
execSSHWithoutPasswd "scp -P ${MASTER_SSH_PORT} ${NOW_DIR}/shell/serverInstall.sh ${MASTER_SSH_USER}@${MASTER_IP}:${K8S_INSTALLER_DIR}" ${MASTER_SSH_PASSWD} & echo "${NOW_DIR}/shell/serverInstall.sh分发完毕"
execSSHWithoutPasswd "scp -P ${MASTER_SSH_PORT} ${COMPILE_TARGET_DIR}/serverDeploy.sh ${MASTER_SSH_USER}@${MASTER_IP}:${K8S_INSTALLER_DIR}" ${MASTER_SSH_PASSWD} & echo "${COMPILE_TARGET_DIR}/serverDeploy.sh分发完毕"
echo "二进制文件分发完毕"
