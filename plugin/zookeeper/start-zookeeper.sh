...
#     --zk_data_dir       存储zookeeper数据的根路径

#     --zk_root_dir       zookeeper的安装目录
...
function create_data_dirs() {
    if [ ! -d ${ZK_DATA_DIR}/data  ]; then
        mkdir -p ${ZK_DATA_DIR}/data
        chown -R $USER:$USER ${ZK_DATA_DIR}/data
    fi

    if [ ! -d ${ZK_DATA_DIR}/datalog  ]; then
        mkdir -p ${ZK_DATA_DIR}/datalog
        chown -R $USER:USER ${ZK_DATA_DIR}/datalog
    fi

    if [ ! -d ${ZK_DATA_DIR}/log  ]; then
        mkdir -p ${ZK_DATA_DIR}/log
        chown -R $USER:$USER ${ZK_DATA_DIR}/log
    fi
...
function create_config() {
    ...
    echo "dataDir=${ZK_DATA_DIR}/data" >> $CONFIG_FILE
    echo "dataLogDir=${ZK_DATA_DIR}/datalog" >> $CONFIG_FILE
...
function create_jvm_props() {
    rm -f $JAVA_ENV_FILE
    echo "ZOO_LOG_DIR=${ZK_DATA_DIR}/log" >> $JAVA_ENV_FILE
...
ID_FILE="${ZK_DATA_DIR}/data/myid"
CONFIG_FILE="${ZK_ROOT_DIR}/conf/zoo.cfg"
LOGGER_PROPS_FILE="${ZK_ROOT_DIR}/conf/log4j.properties"
JAVA_ENV_FILE="${ZK_ROOT_DIR}/conf/java.env"
if [[ $HOST =~ (.*)-([0-9]+)$ ]]; then
    NAME=${BASH_REMATCH[1]}
    ORD=${BASH_REMATCH[2]}
else
...
create_config && create_jvm_props && create_log_props && create_data_dirs && exec ${ZK_ROOT_DIR}/bin/zkServer.sh start-foreground
